                                                ...
                                                tabBox(id='tabset1',width=NULL,
                                                  tabPanel('Visualization',
                                                           fluidRow(
                                                             ...
                                                             column(8,
                                                                    radioButtons(inputId="adjustment",label='Adjustment',choices=c('None'='','July and december'='binary','Seasonal mean curve as offset'='mean_offset','Seasonal mean curve as covariate'='mean_covariate'),selected = '')
                                                             )
                                                           ),
                                                           hr(),
...
server <- function(input, output, session) {
  clicked <- reactiveValues(endpoint_idx=matrix(0,nrow=1,ncol=2))
  ...
  output$plot_FinRegistry <- renderPlot({
    if(nchar(input$endpoint)!=0){
      compare_gam_fits(mod_list=fit_gam()$mod_list,monthly_counts=fit_gam()$dat_endpoint,endpoint=input$endpoint,adjustment=input$adjustment) /
  ...
  output$search_endpoints <- renderUI({
    endpoints=sort(unique(seasonal_summary_FinRegistry$`Endpoint ID`))
    selectizeInput(
      inputId = "endpoint",
  ...
  output$qt_sim_plot <- renderPlot({
    ...
    #seasonal_summary <- run_seasonal_amplitude_mod_qt(dat=sim_dat)
